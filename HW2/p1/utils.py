...
def build_vocabulary(img_paths, vocab_size=400):
    ...
    for path in tqdm(img_paths):
        ...
        _, descriptor = dsift(image, step=[3,3], fast=True)

        features.append(descriptor)

    features = np.concatenate(features, axis=0).astype('float32')

    vocabulary = kmeans(features, vocab_size, initialization="PLUSPLUS")    
...
def get_bags_of_sifts(img_paths, vocab):
    ...
    for path in tqdm(img_paths):
        ...
        dist = cdist(vocab, descriptor, metric='cityblock')

        idx = np.argmin(dist, axis=0)

        histogram, _ = np.histogram(idx, bins=len(vocab))

        histogram_norm = [float(i)/sum(histogram) for i in histogram]

        img_feats.append(histogram_norm)
