...
def plot_learning_curve(logfile_dir, result_lists):
    ...
    # Separate plots for each validation metric if needed
    # Validation Accuracy
    plt.figure(figsize=(10, 5))
    plt.plot(result_lists['val_acc'], label='Validation Accuracy', color='green')
    ...
    # Validation Loss
    plt.figure(figsize=(10, 5))
    plt.plot(result_lists['val_loss'], label='Validation Loss', color='red')
    ...
    for epoch in range(cfg.epochs):
        ...
        with torch.no_grad():
            ...
            for batch, data in enumerate(val_loader):
                sys.stdout.write(f'\r[{epoch + 1}/{cfg.epochs}] Validation batch: {batch + 1} / {len(val_loader)}')
                sys.stdout.flush()
